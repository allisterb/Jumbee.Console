...
/// <remarks>
/// <para>
/// The half-block technique <c>Globe</c> already uses, generalised and given a z-buffer. A sub-pixel is one
/// cell wide and half a cell tall, and a character cell is about twice as tall as it is wide, so sub-pixels come out
/// <em>square</em> — the <c>W × 2H</c> grid is isotropic and shares the same <c>viewport</c> mapping the
...
public sealed class HalfBlockSurface : Control
{
    ...
    /// never a blend of them, so the picture stays on whatever quantised ramp <c>the renderer</c>
    /// produced.
    /// </para>
    /// <para>
