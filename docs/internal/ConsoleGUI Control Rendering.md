...
**Opt-in per-control damage** (`Control.TracksDamage` + `Damage()`) lets a control report sub-rects instead of its whole area. That is a *cost/benefit* decision, not a free win — it narrows what the compositor scans but never what the terminal receives, and the bookkeeping is less cache-friendly than the linear scan it replaces. See [Rendering Model → When damage tracking pays](Rendering%20Model.md#when-damage-tracking-pays) for the measurements and the rule of thumb.
