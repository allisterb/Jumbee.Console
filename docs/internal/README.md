...
- [Scrolling](Scrolling.md) — the frame/control scroll contract and the `IScrollable` interface that replaced it: why the old default silently gave any un-opted-in control a 1000-row scroll range, why scroll-into-view is a `FocusRowChanged` event rather than a polled `FocusRow`, what CS0067 does and does not enforce, and how a frame reveals a focused descendant by walking the drawing-context chain instead of trusting arithmetic a composite would have to maintain.
